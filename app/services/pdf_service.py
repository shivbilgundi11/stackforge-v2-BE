...
def to_html(document: Document) -> str:
    ...
    footer = html.escape(document.share_url or "buildtact.ai")
...
<section class="cover">
  ...
  <div class="stamp">
    Generated by Buildtact on {stamped}<br>
    {footer}
  </div>
</section>
{body}
...
def _render_reportlab(document: Document) -> bytes:
    ...
    story: list[Any] = [
        ...
        Spacer(1, 40 * mm),
        Paragraph(
            f"Generated by Buildtact on {document.stamped_at.strftime('%d %B %Y')}<br/>"
            f"{html.escape(document.share_url or 'buildtact.ai')}",
            styles["stamp"],
        ),
        PageBreak(),
    ]
    ...
    footer_text = document.share_url or "buildtact.ai"
