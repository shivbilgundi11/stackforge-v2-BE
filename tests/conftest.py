...
def pytest_configure() -> None:
    ...
    developer with `ANTHROPIC_API_KEY` in their `.env` would otherwise have
    ...
    moved providers, this list did not, and the architect suite quietly began
    making live calls — which failed the degradation test with
    `hybrid != rule_based` and burned a daily request allowance. So the rule is
    every key, added the same day the setting is.
    """
